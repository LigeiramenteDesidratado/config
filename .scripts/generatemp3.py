...
mhelp = """Give me .cue file
'n' For dry run or 'e' to execute

Example dry run:
    generatemp3.py file.cue n

Example 2:
    generatemp3.py file.cue e
"""
...
for track in tracks:
    ...
    cmd += ' ' + ' '.join('-metadata %s="%s"' % (k, str(v).strip('\"')) for (k, v) in metadata.items())
    cmd += ' -ab 320k -ar 44100 -vn'
    cmd += ' "%.2d - %s.mp3"' % (track['track'], track['title'])
